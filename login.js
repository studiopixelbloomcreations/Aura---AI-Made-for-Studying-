...
(function () {
  ...
  async function init() {
    ...
    try {
      ...
      showToast('Login is unavailable: Firebase config failed to load.', 'error');
      return;
    }
    ...
    if (!auth) {
      showToast('Login is unavailable: Firebase not initialized. Check /api/public-config.', 'error');
      return;
    }
