...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    return json(200, { isCorrect, explanation, score: isCorrect ? 1 : 0, feedback, sessionId: payload.sessionId || payload.session_id || null });
  } catch (error) {
    console.error(JSON.stringify({ at: new Date().toISOString(), fn: "exam_mode_ask_question", error: String(error && error.stack || error) }));
    return json(500, { error: "Aevra could not evaluate that answer right now." });
  }
};
