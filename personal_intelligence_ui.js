(function () {
  ...
  function drawOrb() {
    ...
    const hueShift = (t * 24 + vizRotation * 140) % 360;
    ...
    coreGlow.addColorStop(0, `hsla(${(190 + hueShift * 0.15) % 360}, 100%, 84%, ${0.58 + vizEnergy * 0.4})`);
    coreGlow.addColorStop(0.2, `hsla(${(202 + hueShift * 0.12) % 360}, 100%, 72%, ${0.34 + vizEnergy * 0.35})`);
    coreGlow.addColorStop(0.5, `hsla(${(34 + hueShift * 0.08) % 360}, 100%, 66%, ${0.16 + vizEnergy * 0.25})`);
    ...
    const blobs = [
      { x: cx + Math.cos(t * 0.8) * 140, y: cy - 210 + Math.sin(t * 0.6) * 40, r: 420 + sweep, c1: `hsla(${(198 + hueShift * 0.18) % 360}, 98%, 66%, 0.32)` },
      { x: cx - Math.sin(t * 0.7) * 160, y: cy - 160 + Math.cos(t * 0.5) * 34, r: 400 + sweep * 0.8, c1: `hsla(${(206 + hueShift * 0.1) % 360}, 98%, 64%, 0.28)` },
      { x: cx + Math.sin(t * 0.95) * 180, y: cy - 140 + Math.sin(t * 0.4) * 22, r: 440 + sweep * 0.7, c1: `hsla(${(35 + hueShift * 0.08) % 360}, 98%, 62%, 0.26)` },
      { x: cx + Math.cos(t * 0.55) * 120, y: cy - 120 + Math.sin(t * 0.8) * 28, r: 380 + sweep * 0.75, c1: `hsla(${(192 + hueShift * 0.14) % 360}, 98%, 66%, 0.3)` },
    ];
    ...
    for (let ring = 0; ring < 3; ring += 1) {
      ...
      vizCtx.strokeStyle = `hsla(${(194 + ring * 8 + hueShift * 0.16) % 360}, 100%, 72%, ${0.16 + vizEnergy * 0.32})`;
    ...
    vizCtx.strokeStyle = `hsla(${(34 + hueShift * 0.12) % 360}, 100%, 72%, ${0.2 + vizEnergy * 0.18})`;
    ...
    for (let i = 0; i < vizParticles.length; i += 1) {
      ...
      if (p.prevX || p.prevY) {
        vizCtx.strokeStyle = `hsla(${(190 + (i % 6) * 4 + hueShift * 0.14) % 360},100%,72%,${(0.04 + vizEnergy * 0.16).toFixed(3)})`;
      ...
      const particleHue = i % 8 === 0 ? (34 + hueShift * 0.1) : (194 + (i % 7) * 3 + hueShift * 0.12);
    ...
    for (let k = 0; k < vizFilaments.length; k += 1) {
      ...
      vizCtx.strokeStyle = `hsla(${(197 + hueShift * 0.12 + (k % 5) * 6) % 360},100%,72%,${(f.alpha + vizEnergy * 0.2).toFixed(3)})`;
    ...
    hotCore.addColorStop(0.16, `hsla(${(194 + hueShift * 0.12) % 360}, 100%, 82%, ${0.9})`);
    hotCore.addColorStop(0.48, `hsla(${(35 + hueShift * 0.07) % 360}, 100%, 70%, ${0.5 + vizEnergy * 0.25})`);
